...
if env:
    ...
    p = re.compile(r'(^.*\:\s)|((#|@).*$)|(==.*$)')
    print("'" + "', '".join(sorted(
        set(os.path.basename(p.sub('', f)) for f in cmd))) + "'\n")
...
# avoir un dico persistant pour garder des objets entre deux sessions.
# Pratique quand on a un gros array numpy qu'on n'a pas envie de se faire
# chier à le recréer
class Store(object):
...
try:
    ...
    print('\n/!\ Un session utilisant le store existe déjà.' +
          ' On ne peut pas partager un store.')
